...
class Inference(object):
    ...
    def __init__(self, tf_config=None, min_words=-1, max_sequences=sys.maxsize):
        """
        Parameters
        ----------
        ...
        self.batch_size = self.tf_config['batch_size']
    ...
    def visualize(self, file, tensor_name="item_embedding", proj_name="q2v"):
        ...
        writer = tf.summary.FileWriter(visualization_dir, self.sess.graph)
        proj_config = projector.ProjectorConfig()
        embed = proj_config.embeddings.add()
        embed.tensor_name = tensor_name
        meta_path = os.path.join(visualization_dir, "%s.tsv" % proj_name)
        embed.metadata_path = meta_path
        projector.visualize_embeddings(writer, proj_config)

        concat = self.vectorization(file, meta_path=embed.metadata_path)
        ...
        saver.save(self.sess, os.path.join(visualization_dir, "%s.embs" % proj_name))

    def vectorize(self, file, proj_name="q2v"):
        """
        encode the sequences in file to vector and store in model to visualize them in tensorboard
        Parameters
        ----------
    ...
    def vectorization(self, file, meta_path):
        ...
        vector_concat = np.concatenate(embedding_list, axis=0)
