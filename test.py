...
class Dynamic_RNN:
    ...
    def initialize_rnn_net(self):
        ...
        outputs = tf.reshape(tf.stack(output), [-1, lstm.output_size])
    ...
    def initialize_rnn_optimizer(self):
        ...
        loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=self.preds, labels=self.labels))
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        while step * batch_size < drnn.training_iters:
            ...
            res = sess.run(opt, feed_dict={drnn.seqs: batch_data, drnn.labels: batch_labels, drnn.seqlen: batch_seqlen,
                                           drnn.partitions: partitions})

            if step % 20 == 0:
                l = sess.run(loss,
                             feed_dict={drnn.seqs: batch_data, drnn.labels: batch_labels, drnn.seqlen: batch_seqlen,
                                        drnn.partitions: partitions})
                a = sess.run(acc,
                             feed_dict={drnn.seqs: batch_data, drnn.labels: batch_labels, drnn.seqlen: batch_seqlen,
                                        drnn.partitions: partitions})
        ...
        print "Test accuracy: %.5f" % sess.run(acc, feed_dict={drnn.seqs: test_set.data, drnn.labels: test_set.labels,
                                                               drnn.seqlen: test_set.seqlen,
                                                               drnn.partitions: partitions})