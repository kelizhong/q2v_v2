...
class SaverProcess(mp.Process):
    ...
    def init_writer_handler(self):
        self.raw_handler = tempfile.NamedTemporaryFile(suffix='_%s' % time.strftime("%Y%m%d%H%M%S"), prefix='train_data_raw_',
                                         dir=rawdata_dir,
...
class ProducerProcess(mp.Process):
    def __init__(self, queue, corpus_files, max_num_every_item=8, min_item_length=2, pos_number=3, neg_number=2, info_interval=10000, name="producer"):
